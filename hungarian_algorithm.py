...
class HungarianAlgorithm:
    ...
    def _find_maximum_matching(self) -> List[Tuple]:
        """Find maximum matching in bipartite graph of zeros"""
        zeros = self._find_zeros(self.matrix)
        ...
        # Find matching using augmenting paths
        matching = {}
        
        for i in range(self.n):
            visited = set()
            self._dfs_matching(i, adj, matching, visited)
        
        return [(i, j) for i, j in matching.items()]
    
    def _dfs_matching(self, u: int, adj: Dict, matching: Dict, visited: set) -> bool:
        ...
        for v in adj[u]:
            ...
            if v not in matching or self._dfs_matching(matching[v], adj, matching, visited):
                matching[v] = u
                return True
        return False
    ...
    def _extract_assignment(self) -> List[Tuple]:
        """Extract final assignment from matrix"""
        assignment = []
        used_rows = set()
        used_cols = set()
        ...
        # Greedy assignment (can be improved with Hungarian matching)
        for i, j in zeros:
        ...
        # If not complete, use maximum matching
